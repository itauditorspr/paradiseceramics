...
class BiomtericDeviceInfo(models.Model):
    ...
    sequence_id = fields.Many2one('ir.sequence', string="Sequence", ondelete='cascade', default=lambda self: self.env.ref('hr_attendance_punch.biometric_user_sequence'))
    ...
    # cron_id = fields.Many2one('ir.cron', string="Cron", default= lambda self: self.env.ref('hr_attendance_punch.biometric_attendances').id)
    ...
    def _connect_device(self):
        ...
        try:
            ...
            raise UserError("Connection timed out, make sure the device is turned on and not blocked by the Firewall")
    ...
    def test_connection_device(self):
        ...
        try:
            res = zk.connect()
            if not res:
                raise UserError('Connection Failed to Device ' + str(self.name))
            else:
    ...
    def download_attendance_oldapi(self):
        ...
        bunch_seconds = self.env['ir.config_parameter'].sudo().get_param(
            'hr_attendance_punch.duplicate_punches_seconds')
        ...
        for employee in employees:
            ...
            for log in log_ids.sorted(key=lambda l: l.name):
                ...
                try:
                    ...
                    print(f"Error while creating attendance record: {e}")
                    log.write({
                        'moved': True,
                        'moved_to': attendance_id
                    })
