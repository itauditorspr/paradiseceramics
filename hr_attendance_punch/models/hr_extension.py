...
class hrDraftAttendance(models.Model):
    ...
    employee_id = fields.Many2one(comodel_name='hr.employee', string='Employee',tracking=True, required=True)
        ...
        for rec in self.sorted(key=lambda l: l.name):
            ...
            if rec.attendance_status == 'sign_in':
                vals = {
                    ...
                    'check_in': rec.name,
                    'x_studio_mode': rec.device_id.name,
                ...
                hr_attendance.sudo().write({'check_out': rec.name, 'x_studio_mode_1': rec.device_id.name,})
...
class HrEmployeeBase(models.AbstractModel):
    ...
    last_draft_attendance_id = fields.Many2one('hr.draft.attendance', compute='_compute_last_draft_attendance_id', store=True)
    ...
    def _compute_last_draft_attendance_id(self):
        for employee in self:
            draft_atts = self.env['hr.draft.attendance'].search([('employee_id','=',employee.id)], order='name desc', limit=1)
            employee.last_draft_attendance_id = draft_atts.id if draft_atts else None
